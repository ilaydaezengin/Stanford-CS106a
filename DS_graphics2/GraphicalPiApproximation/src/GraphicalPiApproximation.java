...
public class GraphicalPiApproximation extends GraphicsProgram {
	...
	public void run() {
		...
		for(int i = 0; i <NPOINTS; i++){
			int x = rgen.nextInt(0, RWIDTH);
			int y = rgen.nextInt(0,RHEIGHT);
			GOval point = createPoint(x,y);
			add(point);
			if((CIRCLE_X - x) * (CIRCLE_X - x) + (CIRCLE_Y - y) * (CIRCLE_Y - y) < CIRCLE_RADIUS * CIRCLE_RADIUS){
				inside++;
			}
		}
